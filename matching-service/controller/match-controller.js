    ...
    try {
        const { userOne, userTwo, difficulty, question } = req.body;
            ...
            if (resp.err) {
                ...
                return res.status(201).json({message: `Created new match ${username} successfully!`, user: resp});
            }
    } catch (err) {
...
export async function deleteMatch(req, res) {
    try {
        ...
        return res.status(200).json({message: "Match deleted successfully!", user: resp})
    } catch (err) {
